...
class CircuitCreation extends declared(Widget) {
    ...
    postInitialize() {
        this.ready();
        topic.subscribe("output_relation", (response:any) => {
            this._addGraphicFromTopic(response)
        })
    }
    @property()
    @renderable()
    layersException: any = []; //Массив ID слоев которые не будут входить в идентификацию

    @property()
    @renderable()
    listLinks: any; //Массив объектов которые хранят {name: Имя, alias: Псевдоним, color: Цвет}
    ...
    private open(event:any) {
        ...
        if (event.target.className == CSS.tabLinks) {
            this.arrayLinks.forEach( (element:any) => {
    ...
    private addLinks() {
        let inClass = this;
        inClass.listLinks.forEach(function(parameters:any, i:any) {
        let link = <button
            bind={inClass}
    ...
    private _addGraphicFromTopic(featuresArray:any) {
        let activeLink = this.arrayLinks.find( (item:any) => {
            return item.properties.class == "circuitCreation-tab-links active"
        })
        let colorNow = this.listLinks.find( (item:any) => {
            return item.name == activeLink.properties.id;
        }).color

        let layer = () => {
            let a = this.map.layers["items"];
            let layer;
            for (let i = 0; i < a.length; i++) {
        ...
        featuresArray.forEach( (item:any) => {
            item.forEach( (feature:any) => {
                ...
                this.CircuitLayer.graphics.add(feature)
            })
        })
    }
}
